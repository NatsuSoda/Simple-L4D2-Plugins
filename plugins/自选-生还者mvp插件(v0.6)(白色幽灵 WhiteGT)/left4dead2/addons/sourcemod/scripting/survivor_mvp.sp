...
void displaykillinfected()
{
	...
	for(int i; i < 4; i++)
	{
		...
		tmrf = teamrf[client];
		if(client > 0)
			PrintToChatAll("\x01[统计] \x05%N \x01[ 特感: \x05%d \x01] [ 爆头: \x05%d \x01] [ 丧尸: \x05%d \x01] [ 友伤: \x05%d \x01] [ 被黑: \x05%d \x01]\n", client, sikl, sihd, cikl, tmff, tmrf);
	}
